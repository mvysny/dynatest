...
but it would seem as if the ultimate JUnit's goal was that the test collection must be *pre-known* - computable by analyzing class files alone;
the possibilities to create tests dynamically are severely limited. This approach reduces the possibilities of how to structure test code,
and promotes bad practices:
...
* Reuse of test suites is only possible by the means of inheritance (having a base class with tests, and a bunch of classes
  extending that base class, parametrizing it with constructors). That's a bad practice.
...
and embrace testing structures created programatically.

## Design principles

Ideally, the testing framework should follow these items:

* Promote componentized programming. You should be able to create a test suite as a component,
